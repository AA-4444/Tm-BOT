...
# -------- Base schema (создаёт таблицы, если их нет) --------
DDL = """
...
async def db() -> asyncpg.Pool:
    """
    Создаёт пул и применяет миграцию tsv_all для уже существующих таблиц.
    """
    global _db_pool
    if _db_pool is None:
        ...
        async with _db_pool.acquire() as c:
            ...
            await c.execute(DDL)
            # 🔧 МИГРАЦИЯ: добавим комбинированный RU+EN tsvector и индекс, если их ещё нет
            await c.execute("""
...
async def upsert_message(conn, row):
    # сразу считаем RU+EN tsvector при вставке
    await conn.execute("""
...
async def search_messages(
    ...
    # автодобор источников (фоново), если мало результатов
    if len(items) < 5:
        try:
            async with TelegramClient(StringSession(STRING_SESSION), API_ID, API_HASH) as cli:
            ...
            pass
        asyncio.create_task(crawl_once([q2], limit_msgs=400))
